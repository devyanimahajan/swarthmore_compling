...
if __name__ == "__main__":
	parser = argparse.ArgumentParser(description='calculate precision and recall on a test corpus relative to a hand-annotated corpus (gold standard)')
	...
	parser.add_argument('-i', '--ignore', action='store_true', default=False,
		help='ignore forms in test corpus that aren\'t annotated (as opposed to erroring out)')
	...
	parser.add_argument('test_corpus', help='test corpus (output of transducer)')
	...
	if args.cg and not args.apertium:
		refSents = convertToApertium(args.test_corpus)
		annSents = convertToApertium(args.annotated_corpus)
	elif args.apertium:
		with open(args.test_corpus, 'r') as f_ref:
			refSents = f_ref.read()
	...
	if (len(refTokens) != len(annTokens)):
		...
		if not args.ignore:
			print("ERROR: different number of annotated ({}) and test ({}) cohorts; check these: {}".format(len(annTokens), len(refTokens), ", ".join(extraCohorts)))
			sys.exit()
		else:
	...
	if not args.ignore:
		...
		for (refTok, annTok) in zip(refTokens, annTokens):
			...
			if refLem != annLem:
				print("ERROR: test lemma '{}' doesn't match annotated lemma '{}'".format(refLem, annLem))
				sys.exit()
			else:
		...
		for form in annCohorts:
			if form not in refCohorts:
				print("ERROR: annotated lemma '{}' not in test".format(form))
				sys.exit()
			else:
